...
class BalancesTesting(Strategy):
    ...
    2. создаем 1 ордер и проверяем что гейт отправит нам измененный баланс, и поле used будет не нулевым у
     соответствующего ассета.
    3. ждем пока ордер исполнится (т.е. надо поставить маркет ордер) и проверяем что поле used баланса
    определенного ассета стало нулевым, а активный баланс изменился.
